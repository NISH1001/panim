...
class RainAnimator:
    ...
    def __init__(self, **args):
        ...
        self.ndrops = ndrops
        self.drops = np.zeros(ndrops, dtype=[('position', float, 2),
                                      ('size',     float, 1),
                                      ('growth',   float, 1),
                                      ('color',    float, 4)])
        self.drops['position'] = np.random.uniform(0, 1, (ndrops, 2))
        self.drops['growth'] = np.random.uniform(50, 200, ndrops)
        self.img = self.ax.scatter(self.drops['position'][:, 0], self.drops['position'][:, 1],
                        s=self.drops['size'], lw=0.5, edgecolors=self.drops['color'],
                        facecolors='none')
